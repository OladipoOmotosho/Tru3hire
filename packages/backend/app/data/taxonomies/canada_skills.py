"""
Canadian Skills Taxonomy

This module defines the skill hierarchy for the Canadian Job Market.
Structure: Domain -> Family -> Individual Skills.
...
CANADA_SKILL_HIERARCHY: Dict[str, Dict[str, List[str]]] = {
    "Engineering": {
        "Frontend": [
...
_SKILL_LOOKUP: Dict[str, Tuple[str, str, str]] = {}  # "react" → ("Engineering", "Frontend", "React")
_FAMILY_LOOKUP: Dict[str, Tuple[str, str, List[str]]] = {} # "frontend" → ("Engineering", "Frontend", [...skills])
_DOMAIN_LOOKUP: Dict[str, str] = {} # "engineering" → "Engineering"
...
def find_skill(name: str) -> Optional[Dict[str, Any]]:
    """
    ...
    if name_lower in _SKILL_LOOKUP:
        ...
        siblings = [s for s in CANADA_SKILL_HIERARCHY[domain][family] if s.lower() != name_lower]
        return {
            "level": "skill",
            "value": skill,
    ...
    if name_lower in _DOMAIN_LOOKUP:
        ...
        families = list(CANADA_SKILL_HIERARCHY[domain].keys())
        return {
            "level": "domain",
            "value": domain,
            "parent_chain": [],
